...
//渠道出价 单位：分 [该渠道为客户端bidding模式下，返回渠道出价，非客户端bidding模式下，返回AdSetQDD平台设置的出价]
extern NSString *const kAdSetQDDAdMediaAdnEcpmKey;
//AdSetQDD平台价格 单位：分 [获取该渠道在AdSetQDD平台设置的出价]
extern NSString *const kAdSetQDDAdMediaAdnPlatformPriceKey;
//AdSetQDD平台 fromid [仅支持AdSetQDD平台]
...
//AdSetQDD平台 adCats 为投放行业ID [仅支持AdSetQDD平台]
extern NSString *const kAdSetQDDAdMediaAdCatsKey;
//AdSetQDD平台 adCids 为创意ID [仅支持AdSetQDD平台]
extern NSString *const kAdSetQDDAdMediaAdCidsKey;
//AdSetQDD平台 aderIds为 广告主ID [仅支持AdSetQDD平台]
...
/**C2S bidding上报参数
 ...
 2、AdSetQDD竞价失败上报
 NSDictionary *adInfo = @{kAdSetQDDAdMediaWinPrice:@"竞胜方价格",kAdSetQDDAdMediaLossReason:@"AdSetQDD竞败原因",kAdSetQDDAdMediaWinADN:@"竞胜方渠道ID"}
 kAdSetQDDAdMediaLossReason失败原因从(1：竞争力不足 101：未参与竞价 10001：其他)中按照实际情况填写
 kAdSetQDDAdMediaWinADN渠道ID(1：AdSetQDD其他非bidding广告位 2：第三方ADN 3：自售广告主 4：AdSetQDD其他bidding广告位)中按照实际情况填写
...
//AdSetQDD竞败原因(AdSetQDDMediaLossReason中取值，注意该值务必转为NSString类型)
#define kAdSetQDDAdMediaLossReason @"_LOSS_REASON_"
//AdSetQDD竞胜方渠道ID(AdSetQDDMediaWinADN中取值，注意该值务必转为NSString类型)
...
//AdSetQDD竞胜方渠道ID枚举（即kAdSetQDDAdMediaWinADN值，注意kAdSetQDDAdMediaWinADN值务必转为NSString类型）
typedef NS_ENUM(NSInteger,AdSetQDDMediaWinADN) {
    AdSetQDDMediaWinADN_AdSetQDDNoBidding    = 1,//AdSetQDD其他非bidding广告位
    AdSetQDDMediaWinADN_ThirdADN       = 2,//第三方ADN
    AdSetQDDMediaWinADN_Advertisers    = 3, //自售广告主
    AdSetQDDMediaWinADN_AdSetQDDBidding      = 4 //AdSetQDD其他bidding广告位
};
 ...
 *  1 - 输给AdSetQDD其他非bidding广告位
 *  2 - 输给第三方ADN
 *  3 - 输给自售广告主
 *  4 - 输给AdSetQDD其他bidding广告位
 */
