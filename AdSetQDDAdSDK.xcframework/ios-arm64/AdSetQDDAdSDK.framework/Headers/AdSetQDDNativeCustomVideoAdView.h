//
//  AdSetQDDNativeCustomVideoAdView.h
//  AdSetQDDAdSDK
//
//  Created by AdSetQDD on 2023/4/18.
//  Copyright AdSetQDDAdSDK All rights reserved.
//

#import <UIKit/UIKit.h>
#import "AdSetQDDFeedVideoView.h"

NS_ASSUME_NONNULL_BEGIN

@protocol AdSetQDDNativeCustomVideoAdViewDelegate <NSObject>
//视频播放完成
...
@interface AdSetQDDNativeCustomVideoAdView : UIView
/**
 使用信息流自渲染组件须知
 1、AdSetQDDNativeCustomVideoAdView内部只负责处理添加子控件、绑定/解绑事件，特别说明：AdSetQDDNativeCustomVideoAdView内部不会布局子控件，布局子控件需媒体处理！！！
 ...
 4、接入细节参考demo中【AdSetQDDNativeSimpleVideoAdView控件】
 */
@property(nonatomic,weak)id<AdSetQDDNativeCustomVideoAdViewDelegate> videoDelegate;//播放器播放状态代理
...
@property(nonatomic,strong,readonly) id<AdSetQDDFeedAdMeta> feedAdData;//广告物料
@property(nonatomic,strong,readonly,nullable) UIImageView *iconImageView;//小图icon
@property(nonatomic,strong,readonly) AdSetQDDFeedVideoView *videoView;//视频播放器
...
/**
 ...
 @param iconImageView 小图icon 【AdSetQDDFeedAdMeta中获取不到该参数传空即可】
 @param actionLabel 引导语【AdSetQDDFeedAdMeta中获取不到该参数传空即可】
 @param metaTitleLabel 标题
 @param metaContentLabel 详情/副标题【AdSetQDDFeedAdMeta中获取不到该参数传空即可】
 @param metaSourceLabel 广告来源【AdSetQDDFeedAdMeta中获取不到该参数传空即可】
 */
- (void)loadNativeCustomVideoAdViewWithFrame:(CGRect)frame
                                  feedAdMeta:(id<AdSetQDDFeedAdMeta>)feedAdMeta
                                    metaLogo:(UIImageView *)metaLogo
                                    closeBtn:(UIButton *)closeBtn
                                 videoConfig:(AdSetQDDFeedVideoConfig *)config
...
/**
 ...
 @param metaContentLabel 详情/副标题【AdSetQDDFeedAdMeta中获取不到该参数传空即可】
 @param metaSourceLabel 广告来源【AdSetQDDFeedAdMeta中获取不到该参数传空即可】
 @param metaAppSizeLabel app size【AdSetQDDFeedAdMeta中获取不到该参数传空即可】
 @param metaAppPriceLabel app 价格【AdSetQDDFeedAdMeta中获取不到该参数传空即可】
 @param metaAdxNameLabel adx名称【AdSetQDDFeedAdMeta中获取不到该参数传空即可】
 @param metaAppCommentNumLabel app评论数【AdSetQDDFeedAdMeta中获取不到该参数传空即可】
 @param metaAppDownloadCountDescLabel app下载数【AdSetQDDFeedAdMeta中获取不到该参数传空即可】
 */
- (void)loadNativeCustomVideoAdViewWithFrame:(CGRect)frame
                                  feedAdMeta:(id<AdSetQDDFeedAdMeta>)feedAdMeta
                                    metaLogo:(UIImageView *)metaLogo
...
- (void)resizeAdVideo;
/**
 设置媒体自定义videoView（videoView必须是UIView或其子类）且遵守
 AdSetQDDFeedVideoInterface协议
 注意：
 1、该接口仅在媒体使用【自定义平台功能】时使用
 */
- (void)setMediaCustomFeedVideoView:(UIView<AdSetQDDFeedVideoInterface> *)videoView;
/**
 销毁广告
 */
