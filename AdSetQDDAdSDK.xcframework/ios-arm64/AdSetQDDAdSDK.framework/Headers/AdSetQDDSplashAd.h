...
@interface AdSetQDDSplashAd : AdSetQDDBaseAd
/**
 * 开屏广告基础回调代理对象
 */
@property(nonatomic,weak) id<AdSetQDDSplashAdDelegate> delegate;
/**
 * 开屏广告扩展回调代理对象
 * 如不需要扩展回调相关功能，可不用指定
 */
@property(nonatomic,weak) id<AdSetQDDSplashAdExtensionFuctionDelegate> extDelegate;
/**
 获取广告是否正在显示
 */
 ...
 */
- (void)loadAndShowSplashAdWithPid:(NSString *)pid adParam:(AdSetQDDSplashAdConfigParams *)configParams inWindow:(UIWindow *)window;
/**
 加载广告
 详解：调用该接口加载广告
 注意：如果调用此接口加载广告，请在收到
 adsetqddSplashAdReadySuccess回调后调用show接口
 [@param pid                       广告位id    [必传]
 @param configParams 广告参数   [必传]
 */
- (void)loadSplashAdWithPid:(NSString *)pid adParam:(AdSetQDDSplashAdConfigParams *)configParams;
/**
  渲染广告
 */
- (void)renderAd;
...
/**
 ...
 提示：
 1、注意⚠️为保证开屏广告正常展示，请在收到
 adsetqddSplashAdReadySuccess回调后调用该接口，加载广告方式请务必调用- (void)loadSplashAd:(NSString *)pid;接口
 */
