...
#import "AdSetQDDDrawAdDelegate.h"

NS_ASSUME_NONNULL_BEGIN

@interface AdSetQDDDrawAd : AdSetQDDBaseAd
/**
 DrawAd基础回调代理
 传入时机：加载广告前传入
 */
@property(nonatomic,weak)id<AdSetQDDDrawAdDelegate> delegate;
/**
 DrawAd扩展回调代理
 传入时机：加载广告前传入
 如不需要扩展回调相关功能，可不用指定
 */
@property(nonatomic,weak)id<AdSetQDDDrawAdExtensionFunctionDelegate> extDelegate;
/**
 加载广告
 @param pid             广告位id  必传
 @param adParams  广告参数  必传
 */
- (void)loadAdWithPid:(NSString *)pid adConfigParams:(AdSetQDDDrawAdConfigParams *)adParams;
/**
 展示draw广告
