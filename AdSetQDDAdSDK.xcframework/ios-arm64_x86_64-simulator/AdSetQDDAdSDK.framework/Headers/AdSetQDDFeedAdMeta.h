//
//  AdSetQDDFeedAdMeta.h
//  AdSetQDDAdSDKDev
//
//  Created by AdSetQDD on 2020/7/31.
//  Copyright AdSetQDDAdSDK All rights reserved.
//

#import <Foundation/Foundation.h>

#ifndef AdSetQDDFeedAdMeta_h
#define AdSetQDDFeedAdMeta_h

typedef NS_ENUM(NSInteger, AdSetQDDCreativeType) {
    AdSetQDDCreativeTypeImage      = 1, // 图片
    AdSetQDDCreativeTypeVideo      = 2, // 视频
    AdSetQDDCreativeTypeSmallImage = 11, // 小图
    AdSetQDDCreativeTypeLargeImage = 12, // 大图
    AdSetQDDCreativeTypeThreeImage = 13, // 三图
    AdSetQDDCreativeTypePrerender  = 100000, // 预渲染
};
typedef NS_ENUM(NSInteger, AdSetQDDAdInteractionType) {
    AdSetQDDAdInteractionNormalType = 0, // 普通交互
    AdSetQDDAdInteractionRockType   = 1, // 摇一摇交互
    AdSetQDDAdInteractionTwistType  = 2, // 扭一扭交互
};
@protocol AdSetQDDFeedAdMeta <NSObject>

@required

- (AdSetQDDCreativeType)metaCreativeType;//类型
...
/**
 注意：
 1、metaLogo和metaLogoImageUrl接口互为补充，当其中一个接口未获取到logo，
   可以从另一个接口中获取，两个接口中必有一个接口存在logo
 */
- (UIImageView * _Nullable)metaLogo;//平台Logo

- (NSString * _Nullable)metaLogoImageUrl;//平台Logo url
...
- (AdSetQDDPlatform)metaPlatform;//广告物料归属平台
