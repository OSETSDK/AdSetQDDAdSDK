...
//  AdSetQDDAdSDK
//
//  Created by AdSetQDD on 2020/3/11.
...
@interface AdSetQDDConfig : NSObject
/**
 是否展示广告品牌及广告标识 默认YES
 注意⚠️：此参数一经设置，全局生效即所有广告类型都将被设置(可能存在第三方广告设置无效的情况)
 */
@property(nonatomic,assign,class)BOOL isShowLogo;
/**
 备用字段 示例：[AdSetQDDConfig setExt:@"{\"features\":[{\"key\":\"key名称\",\"value\":[value1,value2]}]}"];
 */
...
/**
 设置禁止投放行业ID
 */
@property(nonatomic,strong,class)NSArray<NSString *> *denyCats;
/**
 设置禁止投放创意ID
 */
@property(nonatomic,strong,class)NSArray<NSString *> *denyCids;
/**
 设置禁止投放广告主ID
 */
 ...
 */
@property(nonatomic,copy,class,getter=getKeywords)NSString *keywords;
/**
  是否使用聚合功能（默认开启聚合功能）
  注意⚠️
 ...
 示例
 [AdSetQDDAdSDK startSDKWithAppid:appid configBlock:^{
     [AdSetQDDConfig setUseMediation:self.model.sdkMediationSwitch];//务必优先设置setUseMediation开关
     [AdSetQDDAdSDK setLogLevel:AdSetQDDLogNone];
     [AdSetQDDConfig setSdkPermissionEnable:YES];
 }];
 */
@property(nonatomic,assign,class,getter=canUseMediation)BOOL useMediation;
/**
 广告主ID
 */
@property(nonatomic,strong,class,getter=getAder_ids)NSArray<NSString *> *ader_ids;
/**
 是否开启设备新标志能力（默认开启 Yes：开启   NO：不开启）
 注意⚠️设置时机：sdk初始化block中调用
 */
@property(nonatomic,assign,class,getter=getSdkPermissionEnable)BOOL sdkPermissionEnable;
/**
 获取用户感兴趣关键词集合
 */
@property(nonatomic,strong,class,readonly,getter=getKeywordArray)NSArray<NSString *> *keywordArray;
/**
 展示视频广告时设置音频category。
...
/**
 ...
 设置时机：sdk初始化时
 */
+ (void)setCustomAdListParams:(NSArray<NSDictionary *> *)adList;
/**
