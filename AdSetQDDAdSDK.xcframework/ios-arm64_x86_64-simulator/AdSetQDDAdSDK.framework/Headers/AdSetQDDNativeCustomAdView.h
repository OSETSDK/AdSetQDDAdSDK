//
//  AdSetQDDNativeCustomAdView.h
//  AdSetQDDAdSDK
//
//  Created by AdSetQDD on 2023/4/17.
...
@interface AdSetQDDNativeCustomAdView : UIView
/**
 使用信息流自渲染组件须知
 1、AdSetQDDNativeCustomAdView内部只负责处理添加子控件、绑定/解绑事件，特别说明：AdSetQDDNativeCustomAdView内部不会布局子控件，布局子控件需媒体处理！！！
 2、AdSetQDDFeedAdMeta中的图片素材需媒体自行加载并添加至iconImageView、imgViews中
 3、以下属性为调用loadNativeCustomAdWithFeedAdMeta接口时传入的控件
 4、接入细节参考demo中【AdSetQDDNativeSimpleImageAdView控件】
 */
 ...
 */
- (void)loadNativeCustomAdWithFeedAdMeta:(id<AdSetQDDFeedAdMeta>)feedAdMeta
